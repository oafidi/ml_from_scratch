...
class RandomForestClassifier:
    def __init__(self, n_estimators: int = 100, max_depth: int = 4, min_samples_leaf: int = 1, min_information_gain: float = 0.0, max_features: int = None):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        for i in range(self.n_estimators):
            root = DecisionTreeClassifier(max_depth=self.max_depth,
            ...
            root.fit(bootstrap_samples_X[i], bootstrap_samples_y[i])
            self.roots.append(root)

    def predict(self, X: np.ndarray) -> np.ndarray:
        votes = np.array([root.predict(X) for root in self.roots]).T
